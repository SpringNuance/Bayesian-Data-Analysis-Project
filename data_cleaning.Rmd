...
sample_rows <- good_rows %>% sample_n(140000 - nrow(bad_rows))
...
# Randomly select 300000 - nrow(bad_rows) rows from good_rows
sample_rows <- good_rows %>% sample_n(40000 - nrow(bad_rows))
...
# For each country, filter train_websites_top_10 and write to separate csv
for (country in countries) {
  train_websites_country <- train_websites_top_10 %>%
    filter(geo_loc == country) %>%
    select(-geo_loc)
  write.csv(train_websites_country, file = paste0("websites/separate/train_website_", country, ".csv"))
}

# Create new dataframe by removing geo_loc column
train_websites_combined <- train_websites_top_10 %>%
  select(-geo_loc)

# Write combined dataframe to csv
write.csv(train_websites_combined, file = "websites/hierarchical/train_website_combined.csv")
