...
```{r}
  separate_sampling <- model_separate$sample(data = stan_data, chains=4, iter_warmup = 1000, iter_sampling = 1000, refresh=0)
```
...
p2 <- ggplot(summaryDiagnostics, aes(x = index, y = ess_bulk)) +
  ...
  annotate("text", x = 17, y = 1800, label = "Samples = 2000", 
           hjust = 1, color = "red") + 
  xlab("Index") +
...
The sampling running options
```{r}
  separate_sampling_prior_sensitivity1 <- model_separate_prior_sensitivity1$sample(data = stan_data, chains=4, iter_warmup = 1000, iter_sampling = 1000, refresh=0)
  separate_sampling_prior_sensitivity2 <- model_separate_prior_sensitivity2$sample(data = stan_data, chains=4, iter_warmup = 1000, iter_sampling = 1000, refresh=0)
```
...
for (k in 1:K){
  ...
  #draws <- pooled_sampling$draws(paste("label_train_pred[",i,"]"), format = "matrix")
  true_train = label_list[[k]][1:N_list[[k]]]
  confusion_matrix <- table(predicted_train, true_train)
  #print(confusion_matrix)
  listTrue <- c(listTrue, true_train)
